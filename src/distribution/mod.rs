...
pub trait DistributionFamily {
    ...
    type Distribution<T: 'static>: Distribution<T, Family = Self>;
}

pub trait Distribution<Value: 'static>:
    MulAssign<f32>
    + From<full::Distribution<Value>>
    + Into<full::Distribution<Value>>
    // + IntoIterator<Item = (Value, f32)>
    + 'static
{
    ...
    #[must_use]
    fn map<T: 'static>(self, fun: impl FnMut(Value) -> T) -> <Self::Family as DistributionFamily>::Distribution<T>;

    #[must_use]
    fn flat_map<T: 'static>(self, fun: impl FnMut(Value) -> <Self::Family as DistributionFamily>::Distribution<T>) -> <Self::Family as DistributionFamily>::Distribution<T>;
